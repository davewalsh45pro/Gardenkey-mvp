...
## Changes
- Dynamic variety/cultivar examples on the tester-create page, based on the plant entered.
- Removed the NFC Quick Guide card and full quick-setup guide from the home page.
- Replaced the visible feedback email address with a single **Send feedback** button.
- Added Aeonium (GK-AEO-001) to the approved plant library using the tester draft and a complete GardenKey care schema.
- Split the former combined Pots & Borders tracker into separate **My Pots** (`pot.html`) and **My Borders** (`border.html`) areas.
- Preserved linking from general plant guides and My Plant records into either saved pots or saved borders.
- Updated visible pilot labels to V25.
