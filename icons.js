...
const GK_ICONS = {
  ...
  Vegetable: `<path d="M50 88 L38 45 Q50 30 62 45 Z" fill="white" fill-opacity=".85"/><path d="M50 45 L50 22" stroke="white" stroke-width="3" stroke-linecap="round"/><path d="M50 30 Q38 20 30 24 M50 26 Q62 16 70 20 M50 22 L50 14" stroke="white" stroke-opacity=".75" stroke-width="3" stroke-linecap="round" fill="none"/>`,
  ...
  Biennial: `<path d="M50 90V45" stroke="white" stroke-width="4" stroke-linecap="round"/>${[24,34,44,54,64,74].map((y,i)=>`<ellipse cx="${50+(i%2?10:-10)}" cy="${y}" rx="7" ry="4" fill="white" fill-opacity=".75" transform="rotate(${i%2?-20:20} ${50+(i%2?10:-10)} ${y})"/>`).join('')}<ellipse cx="50" cy="88" rx="18" ry="7" fill="white" fill-opacity=".55"/>`,
  ...
  Succulent: `${[0,60,120,180,240,300].map(a=>`<ellipse cx="50" cy="50" rx="9" ry="26" fill="white" fill-opacity=".8" transform="rotate(${a} 50 50)"/>`).join('')}<circle cx="50" cy="50" r="8" fill="white" fill-opacity=".95"/>`,
...
function gkIconSvg(category){
  ...
  const keys = Object.keys(GK_ICONS);
  let key = keys.find(k => cat === k.toLowerCase());
  if(!key) key = keys.filter(k => cat.includes(k.toLowerCase())).sort((a,b) => b.length - a.length)[0];
